        ...
        <div class="breadcrumbs-div">
          ...
          <div class="container">
            <ul class="breadcrumb">
              ...
              <li><a class="font-xs color-gray-500" href="#">{{$category -> name}}</a></li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div class="row">
            <div class="col-lg-9 order-first order-lg-last">
              ...
              <div class="box-filters mt-0 pb-5 border-bottom">
                <div class="row">
                <form id="search-form" class="form-search" method="GET" action="{{route('category',$slug)}}">
                  <div class="col-xl-10 col-lg-9 mb-10 text-lg-end text-center"> @if(!isset($limit))<span class="font-sm color-gray-900 font-medium border-1-right span"><?php $current_showing=($products->currentPage() - 1)*$products->perPage()+1; $to_showing=($products->currentpage()-1) * $products->perpage() + $products->count();?> Showing {{($products->currentPage() - 1)*$products->perPage()+1}}&ndash;{{$to_showing}} of {{$products->total()}} results</span>@endif
                      ...
                      <div class="info-right">@if($product->added_by=="seller")<a class="font-xs color-gray-500" href="{{route('shop.visit',$shop_slug)}}">{{trans('Vendor').' '.$shop->name}}</a>@else <span class="font-xs color-gray-500" >{{trans('In House product')}}</span>@endif<br><a class="color-brand-3 font-sm-bold" href="{{route('product.details',$product -> slug)}}">{{$product -> name}}</a>
                       ...
                       <hr>
                        <ul class="list-features">
                          <li>
                          <?php echo (strlen($product->description) <  40) ? $product->description : strip_tags(substr($product->description,0,40)).'...' ;?>
                        </li>
                        </ul>
                      </div>
                    </div>
