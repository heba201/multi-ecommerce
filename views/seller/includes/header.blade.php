...
<?php
$seller_shop=Auth()->user()->shop;
if($seller_shop && $seller_shop->logo !=""){
  $img=asset('assets/images/shops/'.$seller_shop->logo);
}else $img=asset('front/assets/imgs/default.png');
								?>
<nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row default-layout-navbar">
      <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
        <a class="navbar-brand brand-logo" href=""><img src="{{$img}}" alt="logo"/></a>
        <a class="navbar-brand brand-logo-mini" href=""><img src="{{$img}}" alt="logo"/></a>
      </div>
        ...
        <ul class="navbar-nav navbar-nav-right">
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
              ...
              @forelse(Auth::user()->unreadNotifications->take(10)->sortBy('created_at') as $notification)
              @if($notification->type == 'App\Notifications\OrderNotification')
              <div class="dropdown-divider"></div>
                <div class="preview-item-content">
                  <h6 class="preview-subject font-weight-medium">
                    <a href="{{route('orders.show', encrypt($notification->data['order_id']))}}" class="dropdown-item">{{tran('Order code: ')}} {{$notification->data['order_code']}} {{ tran('has been '. ucfirst(str_replace('_', ' ', $notification->data['status'])))}}</a></h6>
              ...
              @endforelse
              <div class="dropdown-divider"></div>
              <a class="dropdown-item preview-item" href="{{ route('seller.all-notification') }}""> 
                <div class="preview-item-content">
