...
@section('content')
    <div class="aiz-titlebar mt-2 mb-4">
      <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{ tran('Shop Verification')}}
                <a href="{{ route('shop.visit', $shop->slug) }}" class="btn btn-link btn-sm" target="_blank">({{ tran('Visit Shop')}})<i class="la la-external-link"></i>)</a>
            </h1>
        </div>
      </div>
    </div>
    <form class="" action="{{ route('shop.verify.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="mb-0 h6">{{ tran('Verification info')}}</h4>
            </div>
            @php
            ...
            <div class="card-body">
                            ...
                            <div class="col-md-10">
                                <div class="custom-file">
                                    <label class="custom-file-label">
                                        <input type="{{ $element->type }}" name="element_{{ $key }}" id="file-{{ $key }}"  required>
                ...
                <div class="text-right mt-4">
                    <button type="submit" class="btn btn-primary">{{ tran('Apply')}}</button>
                </div>
            </div>
        </div>
    </form>
